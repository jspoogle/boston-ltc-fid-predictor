...
try:
    ...
    creds_dict = {
        ...
        "client_id": secret.get("client_id", ""),   # fallback empty string
...
with st.sidebar:
    ...
    st.image("venmo_qr.jpeg", width=220, caption="Scan to donate (100% voluntary)")
    st.caption("Thank you for using and sharing!")

# Header
st.image("header_george.jpg", width='stretch')

# 3-column layout
left_col, main_col, right_col = st.columns([1.2, 7, 1.2])

with left_col:
    st.image("border_left1.jpg", width='stretch')
    st.image("border_left2.jpg", width='stretch')
    st.image("border_left3.jpg", width='stretch')
    st.image("border_left4.jpg", width='stretch')
    st.image("border_left5.jpg", width='stretch')

with right_col:
    st.image("border_right1.jpg", width='stretch')
    st.image("border_right2.jpg", width='stretch')
    st.image("border_right3.jpg", width='stretch')
    st.image("border_right4.jpg", width='stretch')
    st.image("border_right5.jpg", width='stretch')
...
# Indices: 16 for 3/4/26, 18 for 3/10/26
known_indices = [16, 18]
known_inhand_serials = [46085, 46091]  # 3/4/26 and 3/10/26
...
for i, idx in enumerate(known_indices):
    ...
    # Green diamond for in-hand date (higher serial = above)
    fig.add_trace(
        px.scatter(
            x=[sub_ser],
            y=[inhand_ser],
    ...
    # Thin dashed green line from call date (blue) up to in-hand (green)
    fig.add_shape(
        type="line",
        x0=sub_ser,
        y0=fp_ser,
